...
.controller('mainCtrl', function(Auth, $timeout, $location, $rootScope, $window) {
    ...
    $rootScope.$on('$routeChangeSuccess', function() {
        ...
        // determine nav bar
        if (['/','/login','/logout','/register','/shoppingcart','/checkout'].lastIndexOf($window.location.pathname) !== -1){
            app.indexBar = false;
        } else {
            app.indexBar = true;
        }
        // determine intro section
        if (['/','/login','/logout','/register','/appointment','/shoppingcart','/checkout'].lastIndexOf($window.location.pathname) !== -1){
            app.intro = false;
        } else {
            app.intro = true;
        }
